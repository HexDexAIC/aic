#!/usr/bin/env bash
# Start VisionInsert: wait for engine ready in aic_eval_vi, then launch policy.
# Persistent — survives shell session via nohup pattern.

exec >> $HOME/aic_visioninsert_run.log 2>&1
export PATH=$HOME/.pixi/bin:/usr/local/bin:/usr/bin:/bin

log() { echo "[$(date '+%H:%M:%S')] $*"; }
log "Starting VisionInsert run."

while true; do
    if ! docker ps --filter name=aic_eval_vi --format '{{.Names}}' | grep -q aic_eval_vi; then
        log "Container aic_eval_vi not running. Aborting."; exit 1
    fi
...
log "Launching VisionInsert policy..."
